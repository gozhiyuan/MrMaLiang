...
describe("runInit", () => {
  ...
  it("gives the agentic mode the full flagship setup defaults", async () => {
    ...
    // Flagship papers expose Nano Banana to the visual planner by default,
    // while an independent approval gate still prevents implicit spending.
    expect(config.figures.backends.nanobanana).toMatchObject({ enabled: true, requires_approval: true });
