...
describe("compileModeToManifest", () => {
  ...
  it("compiles the agentic mode with a bounded remediation dispatcher", async () => {
    ...
    expect(loop.stages.slice(0, 6).map((stage) => stage.id)).toEqual(["direction_memory", "comparison_opportunities", "artifact_plan", "action_plan", "action_plan_split", "research_action_dispatch"]);
