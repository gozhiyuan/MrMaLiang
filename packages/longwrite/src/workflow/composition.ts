...
import { researchWorkflowProfileDef, type ResearchWorkflowProfile } from "../lib/research/workflow-profiles.js";
import { paperProfile, type PaperProfileId } from "../lib/paper-profiles.js";
...
function withAgenticResearchStages(workflow: Record<string, unknown>, policy?: CompileResearchPolicy, provider: ResearchProviderId = "seed"): Record<string, unknown> {
    ...
    next.stages.splice(refreshedEvidenceIndex + 1, 0,
      ...
      agentStage({
        ...
        instructions: [
          ...
          "For each claim, additionally record modified_object, proposer_or_improver, evaluation_mechanism, persistence_scope, later_use, cross_task_transfer, meta_improvement, human_oversight, sandbox_rollback_provenance, and benchmarks where the full text supports them. Set later_use/cross_task_transfer/meta_improvement to not_reported only after checking the full text for that information, never merely because this excerpt omitted it. At least one of later_use, cross_task_transfer, or meta_improvement is REQUIRED on every A/B-depth claim — leaving all three unset at that depth is not acceptable. Use not_applicable when the source is not itself a self-modifying or self-improving system, so the field genuinely does not apply; use not_reported only when the source is such a system and its full text simply does not state that status.",
        ],
  ...
  if (policy?.semanticScreenEnabled && provider !== "seed") {
    const initialArtifactPlanner = agentStage({
  ...
  const splitActionPlan = scriptStage({
  ...
  const experimentIntent = agentStage({
    id: "experiment_intent",
    title: "Request bounded experimental evidence without executing it",
    owner: "analyst",
    when: "empirical_intent_requested >= 1",
    inputs: ["reviews/artifact-plan.json", "reviews/action-plan.json", "sources/classified_sources.jsonl"],
    outputs: ["experiments/intent.json"],
    instructions: [
      "Write ONLY experiments/intent.json when a validated empirical_pilot artifact intent requires new evidence. It is a request, not an execution plan.",
      "Use the ExperimentIntent schema: version, id, research_question, hypothesis, evidence_role, study_kind, rationale, literature_source_ids, factors, metrics, replication, expected_artifacts, maximum_budget, limitations. Do not include code, commands, credentials, results, or a claim that an experiment ran.",
    ],
    validators: ["required_output_exists"],
    retry: { max_attempts: 2 },
  });
  ...
  next.tool_catalog = [
    ...
    action({
      ...
      instructions: [
        ...
        "Never expose evidence-pipeline bookkeeping as reader-facing prose. In particular, do not call sources or evidence records packets, do not write 'not specified in packet', and do not fill tables with pipeline-status placeholders. Translate evidence boundaries into precise scholarly statements about what the cited work reports, does not evaluate, or leaves unresolved.",
      ],
      validator_commands: [
...
export function compileModeToManifest(
  ...
  const baseWorkflow = isResearchMode(mode)
    ? buildResearchCoreWorkflow(mode, opts.topic, opts.researchProvider ?? "seed", opts.researchPolicy)
    : buildLongformWorkflow(mode);
