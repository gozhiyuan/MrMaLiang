    ...
    const semanticStages: Array<Record<string, unknown>> = [
      ...
      },
      {
        ...
        instructions: [
          ...
          "Use fulltext_priority only for high/medium semantic relevance candidates worth deep reading. Recommend A/B only when the abstract indicates a central/comparative role; final A/B still requires validated full-text evidence.",
        ],
        outputs: ["sources/semantic-screening.json"], validators: ["required_output_exists"], retry: { max_attempts: 2 },
      },
      {
        id: "semantic_screen_repair",
        title: "Validate semantic-screen decisions",
        owner: "analyst", inputs: ["sources/semantic-screening.json", "sources/semantic-screening-candidates.json"],
        outputs: ["reports/semantic-screen-repair.md"], validators: ["required_output_exists"], runtime: "script",
        command: longwriteCommand(["research", "repair-semantic-screen", "."]),
      },
    ];
  ...
  const evidenceRefreshStages: Array<Record<string, unknown>> = policy?.semanticScreenEnabled && provider !== "seed" ? [
    {
      ...
      instructions: [
        ...
        "This is abstract-level semantic triage, not a claim-evidence judgment. Assess only supplied candidate source IDs and configured taxonomy cells. Do not invent claims, quotations, pages, venues, acceptance status, or source IDs. Final A/B depth still requires retrieved full text and validated source evidence.",
      ],
      outputs: ["sources/semantic-screening.json"], validators: ["required_output_exists"], retry: { max_attempts: 2 },
    },
    {
      id: "quality_semantic_screen_repair",
      title: "Validate refreshed semantic-screen decisions",
      owner: "analyst", inputs: ["sources/semantic-screening.json", "sources/semantic-screening-candidates.json"],
      outputs: ["reports/semantic-screen-repair.md"], validators: ["required_output_exists"], runtime: "script",
      command: longwriteCommand(["research", "repair-semantic-screen", "."]),
    },
